    ...
    <header style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px; flex-wrap: wrap; gap: 10px;">
      ...
      <div style="display: flex; align-items: center; gap: 10px;">
        <input 
          v-model="searchId" 
          class="input" 
          placeholder="输入题目ID，如 Q001" 
          style="width: 200px;"
          @keyup.enter="handleSearchById"
        />
        <button @click="handleSearchById" class="btn-primary" :disabled="!searchId">🔍 查询</button>
        <button @click="handleResetSearch" class="btn-cancel" v-if="hasSearched">重置</button>
...
const handleSearchById = async () => {
  ...
  try {
    const res = await getQuestionById(searchId.value.trim());
    if (res.data) {
      list.value = [res.data];
    ...
    if (error.response?.status === 404) {
      alert(`❌ 未找到ID为 "${searchId.value}" 的题目`);
    } else {
